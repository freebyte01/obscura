...
public class Poly{
	...
	void paint( Graphics2D g, double rX, double rY ){
		...
		g.setColor( new Color( 0,0,0,area.activePoly == this 
					&& Obscura.viewer.mapEditMode ? 
							150 
							: 40 ));
		
		if ( Obscura.viewer.mapEditMode 
		&& area.activePoly == this )
		...
		&& points.size() > 1 ){
			...
			for ( Point p : points ){
				...
				if ( lv!=null ){
					...
					if ( Math.abs( c2-ca ) < c21 * 4 / ( lv.length()+v.length()) 
					|| Math.abs( c4-ca ) < c41 * 4 / ( lv.length()+v.length())){
						g.setTransform( t );
			...
			if ( v!=null && lv!=null ){
				double ca = Math.abs( Math.cos( v.angle()-lv.angle()));
				if ( Math.abs( c2-ca ) < 0.1 
				|| Math.abs( c4-ca ) < 0.005 ){
					g.setTransform( t );
