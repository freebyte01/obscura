...
public class ImgDef{
	...
	public Point vect(){
		return vect = pos == null || targ == null ? 
							null 
							: targ.sub( pos ); }
	...
	static ImgDef[] near( double radius, double x, double y, boolean target, ImgDef exclude ){
		...
		res.toArray( resA );
		  Arrays.sort( resA, new Comparator<ImgDef>() {
	...
	public static class Comparison {
		...
		void init(){
			...
			for ( int cmb=0; cmb<cardinality; cmb++ ){
				...
				if ( LOG_COMPARISON )  System.out.println( cmb+"  ->  "+ rels[ cmb ]+" : "+  relsOther[ cmb ]+ " a:"+ Math.round( angles[ cmb ]*1000 )/1000d+" s:"+  Math.round( scales[ cmb ]*1000 )/1000d ); 
			}
	...
	public static final boolean LOG_COMPARISON = false;
	...
	public Comparison[] findSimilarPatterns(){
				...
				if ( otherDef!=this && otherDef.POIs.size()>1 ){ // search all other images which have matching POIs and min. 2 other pois
					...
					if ( samePOIs.size()>1 ){ // we have enough same points, let's calc triangles similarities
						...
						if ( comp.score>0 )
							...
							if ( LOG_COMPARISON ) System.out.println( " **** not similar "+ otherDef.fName );						
					}
		...
		res= res.length > 25 ? Arrays.copyOf( res, 25 ): res; // trim result to max 25 images
	...
	class FragmentComparisonModel implements ComparisonEvaluator{
		...
		public void evaluate( Comparison comp ){
			this.comp = comp;
			if ( LOG_COMPARISON ) System.out.print( "\n matches:" );
			...
			for ( int cmbRef=0; cmbRef<comp.cardinality-1; cmbRef++ ) // now calculate make spectrum which combos are fitting others within certain tolerance
				for ( int cmbOth= cmbRef+1; cmbOth<comp.cardinality; cmbOth++ ){
					...
					if ( dScale < 0.1 ){ 
						comp.scoreScales[ cmbRef ]++; 
						comp.scoreScales[ cmbOth ]++; 
						if ( LOG_COMPARISON ) System.out.print( " s:"+cmbRef+":"+cmbOth+", " );}
					if ( dAngle < 0.01 ){ 
						comp.scoreAngles[ cmbRef ]++; 
						comp.scoreAngles[ cmbOth ]++; 
						if ( LOG_COMPARISON ) System.out.print( " a:"+cmbRef+":"+cmbOth+", " ); }}
...
//				 )
	...
	private Object[] wrapCurrentWinner (
		...
		if ( matchingDef!=null 
			...
			if ( LOG_COMPARISON ) System.out.println( "  *** wrapping as matching "+matchingDef.fName+ " with angle match "+ generalAngle+ " and with scale match "+ generalScale );
...
//						&& cs
	...
	public void paint( Graphics2D g, float opacity ){
		...
		if ( targ != null ){
			...
			if ( this == Obscura.viewer.selectedDef 
			&& vect != null ){
