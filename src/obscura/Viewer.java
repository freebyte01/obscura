...
//	private Container editor;
	...
	static String shortName( String name ) {
		for ( File d : Obscura.OBSERVE_DIRS )
	...
	Font listFont = new Font( "Courier New", Font.PLAIN, 11 );
	Font listFontB = new Font( "Courier New", Font.BOLD, 11 );
	...
	void init() {
		...
		list.setCellRenderer( new ListCellRenderer<File>() {
			...
			public Component getListCellRendererComponent( JList<? extends File> list, File file, int index,
				...
				JLabel c = new JLabel( sdfs.format( new Date( file.lastModified())) 
						+ ( hasThumb ? "[] " : "   " )
						+ ( def!=null?( def.POIs.size()+100+" " ).substring( 1 ).replaceAll( "^0", " " ):"   " )
						+ shortName( file.getPath()).replaceAll( "^.:", "" ).replaceAll( "\\\\", " " ));
		...
		view.addMouseWheelListener( new MouseWheelListener() {
			...
			public void mouseWheelMoved( MouseWheelEvent e ) {
				...
				if ( isMapMode() 
					...
					System.err.println( "zoomed to " + zoom );
		...
		new Thread() { // lazy img reader
			public void run() {
				while ( true ) {
					...
					&& System.currentTimeMillis() > ( imgSelectedTime + ( viewedOrigF != viewedImgF ? 20 : 500 ))
					&& !ImgProvider.REQUESTED_IMAGES.containsKey( selectedImgF.getPath())) { 
						...
						if ( viewedImgF == selectedImgF )
							new Thread(){
								public void run() {
									try {
										selectedImg = ImgProvider.tryProvideImage( viewedOrigF = selectedImgF, false, 0, 0 );
										view.repaint();
							...
							new Thread(){
								public void run() {
									try { 
										alternativeImg = ImgProvider.tryProvideImage( viewedOrigF = alternativeImgF, false, 0, 0 );
										view.repaint();
									} catch ( Exception e ) {  e.printStackTrace(); }}}.start(); }}}
		}.start();
...
//							System.out.println( "clicked on "+ clickSelect+ ":"+ clickedButton );
							new Thread() {
								public void run() {
									try {
										sleep( 100 );
//										System.out.println( "clicked only once : csl: "+ ( clickSelect == null ? "" : clickSelect.file.getAbsolutePath() )+ "   nea: "+ ( nearest == null ? "" : nearest.file.getAbsolutePath() ));
										if ( clickSelect != null ) {
...
//								System.out.println( "clicked on near imag thumb "+ alternativeImgF + " "+ Database.imgInfos.get( alternativeImgF.getName().toLowerCase()));
								
								list.setSelectedValue(
										Database.imgInfos.get( alternativeImgF.getName().toLowerCase()) == null ? 
																	alternativeImgF
																	: Database.imgInfos.get( alternativeImgF.getName().toLowerCase()).file
																, true );
								list.ensureIndexIsVisible( list.getSelectedIndex()); }}}
	...
	void onMouseMove( MouseEvent e ) {
		...
		if (( e.getModifiersEx() & ( b1 | b2 | b3 )) == 0 ) {
			drag = false;
	...
	void selectNearPic() {
		...
		if ( alternativeImgF != null ) {
			...
			if ( alternativeImgF != lastNear ) {
				...
				try {
					// System.out.println( "reading near pic " + nearImgF );
...
//		System.out.println( "key pressed "+ lastKey + " : "+ e.getKeyCode()+ ":"+ e.getKeyChar() );
...
//			System.out.println( "escape pressed" );
		...
		} else if ( ctrl && ( e.getKeyCode() == 'c'
				|| e.getKeyCode() == 'C' )) { // COPY
			
			if ( mapEditMode 
			&& selectedArea != null 
			&& selectedArea.activePoly != null ) {
				polyBuffer = selectedArea.activePoly;
				System.out.println( "polygon copied to buffer " ); }
			
		} else if ( ctrl && ( e.getKeyCode() == 'x' 
				|| e.getKeyCode() == 'X' )) { // CUT
			
			System.out.println( "slide show mode " + slideShow );
			...
			&& selectedArea.activePoly != null ) {
				...
				System.out.println( "polygon moved to buffer" ); }
			
		} else if ( ctrl 
				&& ( e.getKeyCode() == 'v' 
					|| e.getKeyCode() == 'V' )) { // PASTE
			...
			&& polyBuffer != null ) {
				...
				System.out.println( "polygon applied from buffer to selected area" ); }
...
//                    RenderingHints.VALUE_INTERPOLATION_BICUBIC );
			...
			&& !mapEditMode )
				...
				try{ 
					...
					BufferedImage nearestImg= lastNearestF != null 
			...
			if ( viewedImg != null ) {
				...
				System.out.println( "off: "+ imgOff.x +":"+ imgOff.y+ "  "+ vw+":"+vh+ "  "+ mX+":"+mY + " sc: "+ currImgScale);

				g.translate( vw/2 + imgOff.x, vh/2 + imgOff.y ); // center axis to screen
				

				g.setColor( Color.yellow );

				long wt = System.currentTimeMillis();
				g.setTransform( ImgProvider.getAffineForOrientation( 
										g.getTransform(), 
										orientation, 
										(int) Math.round( currRelScale ),
										(int) Math.round( ih * currImgScale )));

				if ( currImgScale < 1 )
					Utils.aaOn( g );

				g.drawImage( viewedImg, 0, 0,
								(int) Math.round( iw * currImgScale ), 
								(int) Math.round( ih * currImgScale ), null ); // scaled actual image draw
//				System.err.println( ".."+ drawCnt++ + " in "+ ( System.currentTimeMillis() - wt )+ "ms" );
				
				Utils.doEllipse( g, -10, -10, 20, 20, false );
				Utils.doEllipse( g, mX -5, mY-5, 10, 10, true );
...
//				Utils.doEllipse( g, -10, -10, 20, 20, true );
//				Utils.doEllipse( g, mX -5, mY-5, 10, 10, true );

				g.setTransform( norm );
				java.awt.geom.Area insideMagnifier = new java.awt.geom.Area(new Ellipse2D.Double( -100 + mX, -100 + mY, 200, 200 ));
				...
				&& drag
					...
					Utils.doEllipse( g, mX - 100, mY - 100, 200, 200, false );
					
					g.setClip( insideMagnifier );

					double magnify = 4;

					g.setTransform( norm );
                    g.translate( 	vw/2 + imgOff.x + ( vw/2 + imgOff.x - mX  )* ( magnify - 1 ) , 
                    				vh/2 + imgOff.y + ( vh/2 + imgOff.y - mY  )* ( magnify - 1 )); // center to screen
                    
//					g.translate( vw/2 + imgOff.x - ( mX - imgOff.x ) * ( magnify - 1 ), 
//								 vh/2 + imgOff.y - ( mY - imgOff.y ) * ( magnify - 1 ) ); // center to screen

                    g.setTransform( ImgProvider.getAffineForOrientation( 
                    									g.getTransform(), 
                    									orientation, 
                    									( int ) Math.round( currRelScale*magnify ),
                    									( int ) Math.round( ih * currImgScale*magnify )));
                    
//					g.setTransform( ImgProvider.getAffineForOrientation( 
//											g.getTransform(), 
//											orientation, 
//											(int) Math.round( magnify ),
//											(int) Math.round( magnify )));
...
//					g.drawImage( viewedImg, 0, 0,	(int) Math.round( iw * currImgScale * magnify ), 
//													(int) Math.round( ih * currImgScale * magnify ), null );
...
//								= poiDef.POIs.get( poi );
								...
								double ppx = ( riw / 2 + p.x * currRelScale ) * magnify;
								double ppy = ( rih / 2 + p.y * currRelScale ) * magnify;
								
								double t;

								switch ( orientation ) {
								case 1: break;
								case 2: ppx = -ppx; break;
								case 3: // PI rotation
								case 4: ppy = -ppy; break;
								case 5: break; // - PI/2 and Flip X
								case 6: break; // -PI/2 and -width
								case 7: break; // PI/2 and Flip
								case 8: // PI / 2
									t = ppy;
									ppy = -ppx;
									ppx = t;
								...
								Utils.doEllipse( (Graphics2D) g, ppx - 8, ppy - 8, 16, 16, true  );

			                    g.setColor( new Color( 0, 0 , 0, 100 ) );

			                    Utils.doLine( (Graphics2D) g, ppx, 0, ppx, ppy );
...
//					Utils.doEllipse( g, -10, -10, 20, 20, false );
//					Utils.doEllipse( g, mX-5, mY-5, 10, 10, false );
...
//					Utils.doEllipse( g, mX - 200, mY - 200, 400, 400, false );
				...
				&& drag
				&& !isMapMode()) {
					java.awt.geom.Area outsideMagnifier = new java.awt.geom.Area( new Rectangle2D.Double( 0, 0, getWidth(), getHeight()));
					outsideMagnifier.subtract( insideMagnifier );
					g.setClip( outsideMagnifier ); }
					

				if ( !drag
				&& !shift
				&& mX > view.getWidth() - 200 ) { 
				...
				if ( shift
				&& !drag ){ // show/edit similarity POIs

					if ( mX < 50 ) 
						showPOImenu= true;

//					System.out.println( poiDef + ":" + alternativeDef );
					...
					if ( Math.abs( mX - p.x ) < 10 
					&& Math.abs( mY - p.y ) < 10 ){
						
						System.out.println( "POI ADD BUTTON" );
...
//									System.out.println( "over chapter "+ ch[ 1 ]);
									selChapter= ch[ 1 ]; }
...
//										System.out.println( "over poi "+ hint );
										overButton = pp + POI_POS; }
									
									g.setColor( active ? 
													disabledMenuBack 
													: lastOverButton == pp || overButton == pp ? 
															overMenuBack 
															: hintMenuBack );
									g.fill( sh );
				...
				if ( selectedDef != null ) {
					...
					if ( alternativeImgF == null 
						&& selectedDef.similar != null
					|| alternativeImgF != null 
						&& selectedDef.isSimilarTo( alternativeImgF.getName().toLowerCase())) {
		...
		if ( viewedImg != null ){ // image name and date title
		...
		int i = 0;
		g.setColor(Color.yellow);
		for ( String s : ImgProvider.REQUESTED_IMAGES.keySet()) { // loading threads
			Utils.doEllipse( g, 5, (i++)*20+10, 5, 5, true ); }
...
//	}
		...
		if ( shift ){
			...
			&& selectedDef.similar != null ){				
				...
				for ( String sim : selectedDef.similar.register ) {
					...
					if ( offY < view.getHeight() 
					&& offY + totalHeight + imgH > 0 )
						g.drawImage( thumb, slideX, offY + totalHeight, stripWidth - 10, imgH, null );
					
					if ( mY > offY + totalHeight -2 
					&& mY < offY + imgH + totalHeight + 2 ) {
	...
	int paintThumbs( Graphics2D g, ImgDef[] thumbsSet, int stripWidth, int slideX, int offY, int totalHeight ) throws IOException{
		...
		if ( thumbsSet != null )
			...
			for ( ImgDef def : thumbsSet ) {
				...
				if ( mY > offY + totalHeight - 2 
				&& mY < offY + imgH + totalHeight+ 2 ) { // specify the alternative image file over which is the mouse cursor
					alternativeImgF = def.file;
					alternativeDef= def;
					selectNearPic();
	...
	public static boolean paintSelector( Graphics2D g, 
			double mX, double mY, double x, double y, double width,
